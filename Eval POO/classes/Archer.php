...
class Archer extends Character {
    ...
    public function arrowDoubleShot(){
        ...
        $status = "$this->name Se prepare a tirer deux fleches !";
        return $status;  
    }
    ...
    public function arrowShot ($target) {
        if ($this->allowCritic && $this->quiver >= 2) {
            ...
            $this->quiver -= 2;
            $status = "$this->name  tire deux flèches sur $target->name !! Degats X2 !! Il reste $target->healthPoints points de vie à $target->name !"; 
    ...
    public function attack($target) {
        ...
        $status = "$this->name attaque avec sa dague $target->name ! Il reste $target->healthPoints points de vie à $target->name !";
        return $status;
    }
